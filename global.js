...
const intersectionalObserver = new IntersectionObserver(callback, options);
...
$(window).on("load", function () { 
	...
	if ($(".body--fade-titles")[0]){
		const targets = document.querySelectorAll(".a-panel__content");
		targets.forEach((panel) => {
			intersectionalObserver.observe(panel);
		});
	}
});
